...
Se puede decir que su periodo es menor o igual $2^{31}-1$.Además no tiene periodo completo, recordemos que de acuerdo a la primera condición del teorema de periodo completo de Hull & Dobell, c y m deben ser primos relativos, en este caso $m=2^{31}-1$ y $c=0$, por lo que no cumple.
...
Mostrar que el promedio de las $U_i$’s tomadas de un ciclo completo de un GLC de periodo completo es $\frac{1}{2}-\frac{1}
{2m}$.
...
tiene un total de $m$ números. Además se probó que 

$$Z_i=a^iZ_0+c\frac{a^i-1}{a-1}\quad \mathrm{mod} \quad m,$$

de manera que el promedio de los $m$ números generados está dado por

$$\frac{1}{m}\sum_{i=1}^{m}\left( a^iZ_0+c\frac{a^i-1}{a-1}\quad \mathrm{mod} \quad m\right)\\
  =-1 + \frac{1}{m}Z_0 \sum_{i=1}^{m} a^i+\frac{c}{m(a-1)}\sum_{i=1}^{m}a^i\quad \mathrm{mod} \quad m\\
  =-1+\left( \frac{Z_0 (a-1)+c}{a-1}\right )\left(\frac{a^{m+1}}{a-1}  \right)\mathrm{mod} \quad m\\
  = \left( -1+\left( \frac{Z_0 (a-1)+c}{a-1}\right )\left(\frac{a^{m+1}}{a-1}  \right)\rigth)-\frac{\left( -1+\left( \frac{Z_0 (a-1)+c}{a-1}\right )\left(\frac{a^{m+1}}{a-1}  \right)\rigth)}{m}\\
  =\left[\frac{1}{m}-1 \rigth] \left[1-\frac{Z_{0}(a^{m+1}-1)}{a-1}-\frac{c(a^{m+1}-1)}{{a-1}^2} \rigth]
$$
...
Un generador de Fibonacci obtiene $X_{n+1}$ a partir de $X_{n}$ y $X_{n−1}$ de la siguiente forma $$X_{i+1}=(X_{i} + X_{i−1}) \quad \mathrm{mod} \quad m\right)$$
donde $X_0$ y $X_1$ están especificados.Supongan que $m = 5$. Sólo dos ciclos son posibles. Encontrarlos, así como su respectivo periodo.

\textbf{Solución:}
Definimos una función con parámetros $n\ge 2$, $x_{0} \ge 0$, $x_1 \ge 1$
```{r,echo=FALSE}
v<-c()
fib<-function(n,x0,x1){
  x<-x0+x1
 for (i in 1:n) {
   v[i]<-((x)/5)-floor((x)/5)
   x<-x+v[i]*5
...
t1<-cbind(fib(10,0,1),fib(10,1,1),fib(10,1,2),fib(10,2,2),fib(10,3,3)
)
t1<-as.data.frame(t1)
colnames(t1)<-c("0-1","1-1","1-2","2-2","3-3")
t1

```
Notamos que tiene periodo 4.

# Problema 9
Genera 10,000 números con una semilla de $Z_0 = 1$ usando el generador $Z_n = 75Z_{n−1}$ mod $2^{31}-1$. Clasifica los números en 10 celdas de igual tamaño y prueben por uniformidad usando la prueba Ji cuadrada con un nivel de confianza del 90 %. Aplicar también la prueba de rachas.
...
```{r}
mye = readLines("C:/Users/rgarciafa/Desktop/digitosE.txt", warn=FALSE)
...
Escriban un programa que utilice el método de la transformación inversa para generar números de la densidad siguiente: $f(x)= \frac{1}{x2} I(x ≥ 1)$. Para probar su programa,hagan un histograma de 10,000 números junto con la densidad f. Verificar la hipótesis de que la muestra sigue la distribución teórica dada y hacer un qq−plot e interpretar.
